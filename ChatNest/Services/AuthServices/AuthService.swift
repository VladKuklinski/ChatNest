...
class AuthService : ObservableObject, AuthServiceProtocol {
    ...
    static let shared = AuthService(activityManager: ActivityStatusManager(), userService: UserService.shared)
    ...
    func deleteAccount() async throws {
        do {
            ...
            if Auth.auth().currentUser == nil{
                print("DEBUG: user was deleted")
            }
            let loginManager = LoginManager()
            loginManager.logOut()
        } catch {
            print("DEBUG: there is an error with deleting an account")
            throw error
        }
    }
    ...
    func sendPasswordReset(email : String) async throws {
        ...
        print("DEGUB: Attempted to send link to email: \(email)")
    ...
    func login(email : String, password : String) async throws {
        do {
            ...
            print("DEBUG: Unable to login - \(error.localizedDescription)")
            throw error
        }
    }
    func createUser(fullName: String, email : String, password: String, profileImage: UIImage?) async throws {
        do {
            ...
            print("DEBUG: Unable to create user - \(error.localizedDescription)")
            throw error
        }
    }
    ...
    func setUserSession(_ user: FirebaseAuth.User?) async {
        ...
        do {
            try await userService.fetchUserData(uid: uid)
            activityManager.setupOnlineStatus(for: uid)
            print("DEBUG : User was fetch")

        } catch {
            print("DEBUG : Failed too fetch a user - \(error.localizedDescription)")
        }
